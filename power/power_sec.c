...
static void power_set_interactive(struct power_module __unused * module, int on)
{
	...
	if (current_power_profile == PROFILE_NORMAL)
		interactive_set_profile(on ? PROFILE_NORMAL : PROFILE_POWER_SAVE);
	else if (current_power_profile == PROFILE_HIGH_PERFORMANCE)
		interactive_set_profile(on ? PROFILE_HIGH_PERFORMANCE : PROFILE_POWER_SAVE);
...
static void sec_power_hint(struct power_module *module, power_hint_t hint, void *data)
{
	...
	switch (hint) {
		...
		case POWER_HINT_DISABLE_TOUCH:
			set_input_device_state(data ? 1 : 0);
			break;
